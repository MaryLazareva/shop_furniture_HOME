...
from app1.orders.models import Order, OrderItem
...
def registration(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"{user.username}, Вы успешно зарагистрированы и вошли в аккаунт",
            )
            return HttpResponseRedirect(reverse("main:index"))
    else:
...
def profile(request):
    if request.method == "POST":
        ...
        )  # используем instance, для какого пользователя нужно сохранять информацию
        if form.is_valid():
            form.save()
